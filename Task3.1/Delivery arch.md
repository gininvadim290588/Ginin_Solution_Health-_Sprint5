...
Текущий поток LIS → CRM ненадёжен: FTP падает с 425, файлы дублируются, REST-коннектор не выполняет корректный retry, а задержка передачи может достигать двух часов. Это подтверждается Jira INT-002, INT-008, INT-011 и Service Desk SD-005.
